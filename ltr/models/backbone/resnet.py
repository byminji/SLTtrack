...
class ResNet123(Backbone):
    ...
    def __init__(self, block, layers, output_layers, num_classes=1000, inplanes=64, dilation_factor=1, frozen_layers=()):
        ...
        super(ResNet123, self).__init__(frozen_layers=frozen_layers)
...
def resnet50_123(output_layers=None, pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    """
    ...
    model = ResNet123(Bottleneck, [3, 4, 6, 3], output_layers, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']), strict=False)
    return model