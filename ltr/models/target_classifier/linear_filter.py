...
class LinearFilter(nn.Module):
    ...
    def forward(self, train_feat, test_feat, train_label, train_bb, *args, **kwargs):
        ...
        # Extract features
        train_feat = self.extract_classification_feat(train_feat, num_sequences)    # 3 x 10 x 512 x 22 x 22
        test_feat = self.extract_classification_feat(test_feat, num_sequences)      # 3 x 10 x 512 x 22 x 22

        # 
        encoded_feat, decoded_feat = self.transformer(train_feat, test_feat, train_label)

        encoded_feat = encoded_feat.reshape(-1, num_sequences, *encoded_feat.shape[-3:])    # 3 x 10 x 512 x 22 x 22
        decoded_feat = decoded_feat.reshape(-1, num_sequences, *decoded_feat.shape[-3:])    # 3 x 10 x 512 x 22 x 22
    ...
    def get_filter(self, feat, bb, *args, **kwargs):
        ...
        weights = self.filter_initializer(feat, bb) # 10(sequence) x 512 x 4 x 4
