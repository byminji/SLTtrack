...
class DiMPnet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, train_label, test_proposals, *args, **kwargs):
        ...
        # train_imgs/test_imgs : 3(images) x 10(sequences) x 3 x 352(H) x 352(W)
        # train_bb : 3x10x4
        ...
        train_feat = self.extract_backbone_features(train_imgs.reshape(-1, *train_imgs.shape[-3:])) # layer2, layer3
        test_feat = self.extract_backbone_features(test_imgs.reshape(-1, *test_imgs.shape[-3:]))    # layer2, layer3

        # Classification features
        train_feat_clf = self.get_backbone_clf_feat(train_feat) # 30 x 1024 x 22 x 22 (l3)
        test_feat_clf = self.get_backbone_clf_feat(test_feat)   # 30 x 1024 x 22 x 22 (l3)
        ...
        # target_scores: list(6 = result of initial filter & each optim iteration (5)) of Tensor(3x10x23x23)
        target_scores = self.classifier(train_feat_clf, test_feat_clf, train_label, train_bb, *args, **kwargs)

        # Get bb_regressor features
        train_feat_iou = self.get_backbone_bbreg_feat(train_feat) # list [Tensor(30x512x44x44), Tensor(30x1024x22x22)] (l2, l3)
        test_feat_iou = self.get_backbone_bbreg_feat(test_feat)   # list [Tensor(30x512x44x44), Tensor(30x1024x22x22)] (l2, l3)

        # Run the IoUNet module
        iou_pred = self.bb_regressor(train_feat_iou, test_feat_iou, train_bb, test_proposals) # 3x10x128
