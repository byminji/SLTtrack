...
from ..utils import overlap_ratio, success_overlap, success_error, success_overlap_vot
from toolkit.utils.region import vot_overlap_traj
...
class OPEBenchmark:
    ...
    # def convert_corner_to_bb(self, bboxes):
    ...
    def eval_overlap(self, eval_trackers=None):
        """
        Args: 
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                overlap_ret_[video.name] = overlap_ratio(gt_traj, tracker_traj)
            overlap_ret[tracker_name] = overlap_ret_
        return overlap_ret

    def eval_overlap_vot(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                overlap_ret_[video.name] = vot_overlap_traj(gt_traj, np.squeeze(tracker_traj))
            overlap_ret[tracker_name] = overlap_ret_
        return overlap_ret
    ...
    def eval_success_vot(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                success_ret_[video.name] = success_overlap_vot(gt_traj, np.squeeze(tracker_traj), n_frame)
            success_ret[tracker_name] = success_ret_
        return success_ret

    def eval_success_threshold(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                auc = success_overlap(gt_traj, tracker_traj, n_frame)
                op50_ret_[video.name] = auc[10]
                op75_ret_[video.name] = auc[15]
            op50_ret[tracker_name] = op50_ret_
            op75_ret[tracker_name] = op75_ret_
        return (op50_ret, op75_ret)

    def eval_success_threshold_vot(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                auc = success_overlap_vot(gt_traj, np.squeeze(tracker_traj), n_frame)
                op50_ret_[video.name] = auc[10]
                op75_ret_[video.name] = auc[15]
            op50_ret[tracker_name] = op50_ret_
            op75_ret[tracker_name] = op75_ret_
    ...
    def show_result(self, success_ret, precision_ret=None,
            norm_precision_ret=None, overlap_ret=None, op50_ret=None, op75_ret=None, show_video_level=False, helight_threshold=0.6):
        """pretty print result
        Args:
        ...
        header = ("|{:^"+str(tracker_name_len)+"}|{:^9}|{:^16}|{:^11}|{:^11}|{:^11}|{:^11}|").format(
                "Tracker name", "Success", "Norm Precision", "Precision", "Overlap", "OP50", "OP75")
        formatter = "|{:^"+str(tracker_name_len)+"}|{:^9.3f}|{:^16.3f}|{:^11.3f}|{:^11.3f}|{:^11.3f}|{:^11.3f}|"
        print('-'*len(header))
        print(header)
        print('-'*len(header))
        for tracker_name in tracker_names:
            ...
            if norm_precision_ret is not None:
                ...
                norm_precision = 0
            if overlap_ret is not None:
                episodes = list(overlap_ret[tracker_name].values())
                ep_len = len(episodes)
                iou_list = []
                for i in range(ep_len):
                    iou_list.append(np.mean(episodes[i]))
                overlap = np.mean(iou_list)
#                overlap = np.mean(list(overlap_ret[tracker_name].values()))
            else:
                overlap = 0
            if op50_ret is not None:
                op50 = np.mean(list(op50_ret[tracker_name].values()))
            else:
                op50=0
            if op75_ret is not None:
                op75 = np.mean(list(op75_ret[tracker_name].values()))
            else:
                op75=0

            print(formatter.format(tracker_name, success, norm_precision, precision, overlap, op50, op75))
        print('-'*len(header))
