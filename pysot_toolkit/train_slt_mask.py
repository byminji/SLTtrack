...
from pysot.datasets.sequence_mask_dataset import SequenceDataset
...
parser.add_argument('--expr', type=str, default='slt_siamrpnpp',
...
def explore(tracker, batch_video):
    ...
    Args:
        ...
        batch_video: The input data, should contain the fields 'images', 'gt_orig', 'template', 'template_bbox'.
            ...
            - masks: Binary mask image. list (num_seq) of array (num_frames, h, w)
            - mask_weight
    ...
    Returns:
        Dictionary of exploration results
            ...
            - penalty: Aspect ratio penalty calculated during inference. Tensor (num_frames-1, num_seq, 3125)
    """
    ...
    masks = batch_video['masks']
    mask_weight = batch_video['mask_weight']
    ...
    for idx in range(np.max(num_frames)):
        ...
        here_masks = [mask[idx] for mask in masks]
        here_masks = here_masks + here_masks
        ...
        if idx == 0:
            ...
            with torch.no_grad():
                outputs = tracker.batch_track(here_images, here_gt_bbox, here_masks, action_mode='half')
            ...
            mask_list.append(outputs['m_crop'][num_seq:])
            anchor_list.append(outputs['anchor'][num_seq:])
            gt_in_crop_list.append(outputs['gt_in_crop'][num_seq:])
    ...
    return {'z_crop': z_crop[num_seq:],
            ...
            'm_crop': torch.stack(mask_list),
            'mask_weight': torch.from_numpy(np.array(mask_weight)),
            'anchor': torch.stack(anchor_list),
            'gt_in_crop': torch.stack(gt_in_crop_list),
...
def train(tracker, dataloader, optimizer, lr_scheduler):
    ...
    for step, batch in enumerate(dataloader):
        ...
        m_crop = explore_result['m_crop'].view(-1, 255, 255)
        mask_weight = explore_result['mask_weight'].unsqueeze(1).repeat(1, num_frames).view(-1)
        anchor = explore_result['anchor'].view(num_seq * num_frames, *explore_result['anchor'].size()[2:])
        gt_in_crop = explore_result['gt_in_crop'].view(num_seq * num_frames, *explore_result['gt_in_crop'].size()[2:])
        ...
        while cursor < num_samples:
            ...
            if cfg.REFINE.REFINE:
                model_inputs['anchor_center'] = anchor[cursor:cursor + cfg.TRAIN.BATCH_SIZE]
                model_inputs['gt_in_crop'] = gt_in_crop[cursor:cursor + cfg.TRAIN.BATCH_SIZE]
            if cfg.MASK.MASK:
                model_inputs['mask_in_crop'] = m_crop[cursor:cursor + cfg.TRAIN.BATCH_SIZE]
                model_inputs['mask_weight'] = mask_weight[cursor:cursor + cfg.TRAIN.BATCH_SIZE]
            ...
            # Bounding bos refinement loss
            if cfg.REFINE.REFINE:
                refine_loss = outputs['refine_loss'].mean() * (bs / num_samples)
            else:
                refine_loss = torch.tensor(0)

            # Mask prediction loss
            if cfg.MASK.MASK:
                mask_loss = outputs['mask_loss'].mean() * (bs / num_samples)
            else:
                mask_loss = torch.tensor(0)

            final_loss = cfg.TRAIN.CLS_WEIGHT * cls_loss + cfg.TRAIN.LOC_WEIGHT * reg_loss \
                        + cfg.TRAIN.REFINE_WEIGHT * refine_loss + cfg.TRAIN.MASK_WEIGHT * mask_loss
            final_loss.backward()

            reg_loss_sum += reg_loss.item()
            loss_sum += cls_loss.item()
            refine_loss_sum += refine_loss.item()
        ...
        print('[%s][%d epoch][%d step] loss %.4f, reg_loss: %.4f, refine_loss: %.4f, mask_loss: %.4f, reward: %.4f, b_miou: %.4f, e_miou: %.4f, elapsed_time: %.1f,explore_fps: %.1f' %
           (args.expr, epoch + 1, step, loss_sum, reg_loss_sum, refine_loss_sum, mask_loss_sum,
            np.mean(reward_record), np.mean(miou_record), np.mean(e_miou_record), batch_time - data_time,
            explore_fps))
        ...
        log = {
           ...
           'refine_loss': refine_loss_sum,
           'mask_loss': mask_loss_sum,
