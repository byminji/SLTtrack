...
from pysot.models.loss import select_cross_entropy_loss, weight_l1_loss, det_loss_smooth_l1, mask_loss_bce
...
class ModelBuilder(nn.Module):
    ...
    def forward(self, data):
        ...
        if 'anchor_center' in data and 'gt_in_crop' in data and 'mask_in_crop' in data and 'mask_weight' in data:
            anchor = data['anchor_center']
            gt_bboxes = data['gt_in_crop']
            search_mask = data['mask_in_crop']
            mask_weight = data['mask_weight']

            # num_batch = anchor.size()[0]
            # batch_select = torch.arange(num_batch).cuda()
            batch_select = (~(gt_bboxes.sum(dim=1).data.eq(0))).nonzero().squeeze()
            num_batch = batch_select.size()[0]
            search_mask = torch.index_select(search_mask, 0, batch_select)
            mask_weight = torch.index_select(mask_weight, 0, batch_select)
            ...
            refine_loss = det_loss_smooth_l1(bbox_refine, regression_target)

            # mask_weight = (~(search_mask.sum(dim=(1,2)).data.eq(0))).int()
            mask_loss = mask_loss_bce(mask_pred, mask_targets, mask_weight)
