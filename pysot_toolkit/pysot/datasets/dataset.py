...
from pysot.utils.bbox import center2corner, corner2center, Center, Corner
...
class TrkDataset(Dataset):
    ...
    def _get_bbox(self, image, shape):
        ...
        if cfg.DATASET.PERTURB.PERTURB:
            bbox = self._data_perturb(bbox, scale_z, image.shape[:2])
        return bbox

    def _data_perturb(self, bbox, scale, boundary):
        # bbox : x1 y1 x2 y2
        ...
        # Add noise to bbox
        cx = cx + np.random.normal(mu, sigma)*scale
        cy = cy + np.random.normal(mu, sigma)*scale
        w = w + np.random.normal(mu, sigma)*scale
        h = h + np.random.normal(mu, sigma)*scale
    ...
    def __getitem__(self, index):
        ...
        # get labels
        cls, delta, delta_weight, anchor_center, overlap = self.anchor_target(
