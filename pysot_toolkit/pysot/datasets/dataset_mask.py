...
from pysot.datasets.augmentation_mask import Augmentation
...
class SubDataset(object):
    def __init__(self, name, root, anno, frame_range, num_use, start_idx):
        ...
        self.has_mask = name in ['COCO', 'YTVOS']
        self.path_format = '{}.{}.{}.jpg'
        self.mask_format = "{}.{}.m.jpg"
...
class TrkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if dataset.has_mask and not neg:
            mask = cv2.imread(search[2], 0)
            if mask is not None:
                search_mask = (mask > 0).astype(np.float32)
        ...
        refine_weight = 0 if neg else 1 # pos pair / neg pair
        mask_weight = 1 if dataset.has_mask else 0

        return {
                'template': template,
                'search': search,
                'search_mask': np.array(mask, np.float32),
                'mask_weight': np.array(mask_weight, np.float32),
                'bbox_weight': np.array(refine_weight, np.float32),
