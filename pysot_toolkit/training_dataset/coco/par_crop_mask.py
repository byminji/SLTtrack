...
def crop_img(img, anns, set_crop_base_path, set_mask_base_path, instanc_size=511):
    img_name = img['file_name'].split('/')[-1].split('.')[0]
    frame_crop_base_path = join(set_crop_base_path, img_name)
    ...
    for trackid, ann in enumerate(anns):
        ...
        mask = cv2.imread('{}/{}/000000_{:02d}.png'.format(set_mask_base_path, img_name, trackid))
        m = crop_like_SiamFC(mask, bbox, instanc_size=instanc_size)
        cv2.imwrite(join(frame_crop_base_path, '{:06d}.{:02d}.m.jpg'.format(0, trackid)), m)
...
def main(instanc_size=511, num_threads=12):
    ...
    for dataType in ['train2017']: #['val2017', 'train2017']:
        ...
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [executor.submit(crop_img, coco.loadImgs(id)[0],
                                  ...
                                  set_crop_base_path, set_mask_base_path, instanc_size) for id in coco.imgs]
